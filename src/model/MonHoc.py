...
class MonHoc(db.Model):
    ...
    def save_monhoc(mh_id, mh_maso, mh_ten, mh_sotinchi,mh_lythuyet,mh_thuchanh):
        ...
        if existing_monhoc and (str(existing_monhoc.mh_id) != str(mh_id)):
            existing_monhoc.mh_id = mh_id
            existing_monhoc.mh_maso = mh_maso
            existing_monhoc.mh_ten = mh_ten
            existing_monhoc.mh_sotinchi = mh_sotinchi
            existing_monhoc.mh_lythuyet = mh_lythuyet
            existing_monhoc.mh_thuchanh = mh_thuchanh
            db.session.commit()
        else:
            print("có thể saved")
